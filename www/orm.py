...
async def select(sql, args, size=None):
    ...
    async with __pool.get() as conn:  #打开pool的方法,或-->with await __pool as conn:
...
#封装INSERT,UPDATE,DELETE,我一开始的做法
...
class ModelMetaclass(type):
    def __new__(cls, name, bases, attrs):
        ...
        tableName, ', '.join(escaped_fields), primaryKey, create_args_string(len(escaped_fields) + 1)) #占位符
        attrs['__update__'] = 'update `%s` set %s where `%s`=?' % (
        tableName, ', '.join(map(lambda f: '`%s`=?' % (mappings.get(f).name or f), fields)), primaryKey) #查询列的名字，也看一下在Field定义上有没有定义名字，默认None
